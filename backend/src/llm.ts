...
export type LlmResult = { parsed: unknown | null; rawText: string };

function mustGet(name: string, v: string | undefined) {
  if (!v) throw new Error(`缺少环境变量 ${name}，无法调用大模型`);
  return v;
}
...
export async function summarizeWeeklyReport(config: AppConfig, input: { htmlText: string; pdfText: string }) {
  ...
  try {
    ...
    if (res.status === 404) res = await doFetch(endpoint2);

    const bodyText = await res.text();
    if (!res.ok) {
      if (res.status === 400 && bodyText.includes("Model Not Exist")) {
        throw new Error(
          `大模型接口错误 400：Model Not Exist（请检查 LLM_MODEL）。DeepSeek 通常用 deepseek-chat 或 deepseek-reasoner；当前=${model}`,
        );
      }
    ...
    return { parsed, rawText } satisfies LlmResult;
  } finally {
    clearTimeout(t);
  }
}
